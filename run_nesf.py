...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    near = 0.1 * far
    near, far = near * torch.ones_like(rays_d[...,:1]), far * magnitude_d
    ...
    k_extract = ['trans_map', 'disp_map', 'acc_map', 'depth_map']
    ret_list = [all_ret[k] for k in k_extract]
    ret_dict = {k : all_ret[k] for k in all_ret if k not in k_extract}
    return ret_list + [ret_dict]


def render_path(hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, far=10.,near=0.):
    ...
    for i in range(num):
        ...
        rz = ro + dr*i
        rays_o, rays_d = get_rays_euler(H,W, focal, [0., 0.,rz-np.pi], [far/2*np.cos(rz), far/2*np.sin(rz), 0])
        # print(f"rays_d {rays_d.shape}")
        rays = [torch.Tensor(rays_o).to(device),torch.Tensor(rays_d).to(device)]
        
        trans, disp, acc, depth, _ = render(H, W, K, chunk=chunk, rays=rays, **render_kwargs)
...
def create_nerf(args):
    ...
    model = NeSF(D=args.netdepth, W=args.netwidth,
    ...
    if args.N_importance > 0:
        model_fine = NeSF(D=args.netdepth_fine, W=args.netwidth_fine,
                          input_ch=input_ch, output_ch=output_ch,).to(device)
        for m in model_fine.modules():
            if isinstance(m,(nn.Linear)):
                nn.init.xavier_uniform_(m.weight)
                m.bias.data.zero_()
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    trans = 1. - torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)), 1.-alpha + 1e-10], -1), -1)[:, :-1]
    trans_map = trans[:,-1]
    ...
    return trans_map, disp_map, acc_map, weights, depth_map
...
#     raw = run_network(pts)
    ...
    trans_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)

    if N_importance > 0:

        trans_map_0, disp_map_0, acc_map_0 = trans_map, disp_map, acc_map
...
#         raw = run_network(pts, fn=run_fn)
    ...
    ret = {'trans_map' : trans_map, 'disp_map' : disp_map, 'acc_map' : acc_map, 'depth_map': depth_map}
    if retraw:
        ret['raw'] = raw
    if N_importance > 0:
...
def config_parser():
    ...
    parser.add_argument("--datadir", type=str, default='./data/001', 
...
def train():
    ...
    if use_batching:
        ...
        print('get rays')
        rays = np.stack([get_rays_np_wall(H, W, focal, location, wall_location, scale=1.) for location in locations], 0) # [N, ro+rd, H, W, 3]
        print('done, concats')
    ...
    for i in trange(start, N_iters):
        ...
        trans, disp, acc, depth, extras = render(H, W, K, chunk=args.chunk, rays=batch_rays,
        ...
        optimizer.zero_grad()
        trans_loss = img2mse(trans, target_trans_s)
        # trans = extras['raw'][...,-1]
        

        psnr = mse2psnr(trans_loss)


        trans_loss0 = img2mse(extras['trans0'], target_trans_s)
        loss = trans_loss + trans_loss0
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            with torch.no_grad():
                render_path(hwf, K, args.chunk, render_kwargs_test, gt_imgs=images[i_test], savedir=testsavedir,far=far,near=near)
            # with torch.no_grad():
        ...
        if i%args.i_video==0 and i > 0:
            ...
            imageio.mimwrite(moviebase + 'depth.mp4', to_rgb(depths), fps=30, quality=8)
